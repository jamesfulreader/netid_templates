...
const adobeEmailInput = document.getElementById("adobeEmail");
const previewEmail = document.getElementById("previewEmail");
const insertedEmail = document.getElementById("insertedEmail");

function updateAdobePreview() {
    const value = adobeEmailInput.value.trim() || "user@unm.edu";
    previewEmail.textContent = value;
    insertedEmail.textContent = value;
}

adobeEmailInput.addEventListener("input", updateAdobePreview);
updateAdobePreview();

document.getElementById("copyAdobeSubject").addEventListener("click", function () {
    const subject = "Adobe Acrobat Sign Account Update for " + (adobeEmailInput.value.trim() || "user@unm.edu");
    navigator.clipboard.writeText(subject).then(() => alert("Subject copied to clipboard!"));
});

document.getElementById("copyAdobeBody").addEventListener("click", function () {
    const bodyText = document.getElementById("previewBody").innerText;
    navigator.clipboard.writeText(bodyText).then(() => alert("Body copied to clipboard!"));
});